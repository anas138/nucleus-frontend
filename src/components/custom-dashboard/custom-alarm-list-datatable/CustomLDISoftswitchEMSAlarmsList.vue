<template>
  ...
  >
    <template v-slot:severity="{ row: alarm }">
      <span v-html="LDISoftswitchEMSAlarmSeverityBadge(alarm.severity)"></span>
    </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const fetchData = async () => {
      ...
      const res = await LDISoftswitchEMSAlarmService.getAllAlarms(
    ...
    onUnmounted(() => {
      if (props.socketToggled) {
        SocketIoService.socket.off(
          AppConstants.SOCKET_EVENTS.LDI_SOFTSWITCH_EMS_ALARMS,
          eventListenerCB
        );
      }
    });
