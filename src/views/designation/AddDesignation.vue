...
import { getDesignationFormConfig } from "./designation-form-config";
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      formConfig.value = await getDesignationFormConfig();
    });
    async function submit(data) {
      const response = await CurdApiService.createResource(data, "designation");
      AlertForCreate(response, "designation");
    }
    return {
      formConfig,
      submit,
