  ...
  <crud-view-layout :key="refreshKey">
    <template #actions>
      <button
        ...
        @click="syncDevices"
        v-if="$hasPermission(permissions.nceGpon.devicesSync)"
      >
    ...
    <template v-slot:body>
      ...
      >
        <template v-slot:created_on="{ row: device }">
          {{ $format.datetimeFormat(device.created_on) }}
        </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function fetchData() {
      return await NceGponService.getAllDevices();
    }
    async function syncDevices() {
      NceGponService.syncDevices().then((response) => {
        ElNotification({
          type: "success",
          message: response,
        });
