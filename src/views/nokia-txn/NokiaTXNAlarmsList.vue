<template>
  <crud-view-layout>
    <template v-slot:title-help>
      <span class="d-flex justify-content-center align-items-center">
        ...
        (Live Alarms from {{ $nokiaTXNPresentationName }} Monitoring NMS)
        <button
      ...
      <div class="row">
        <div :class="{ 'col-md-8 border-end': rowAlarmId > 0 }">
          <KTDataTable
            ...
            :filterOptions="NceTXNFilterConfig"
          ...
          >
            <template #custom-actions>
              <TableColumnFiltration
                ...
                :tableColumnFiltrationKey="
                  AppConstants.TABLE_FILTER_COLUMN_KEYS.NOKIA_TXN_ALARM_LIST
                "
            ...
            <template v-slot:actions="{ row: alarm }">
              <div class="d-flex gap-2">
                ...
                <span
                  ...
                  @click="
                    $router.push(
                      `/trouble-ticket/${AppConstants.APP_TYPE.NOKIA_TXN.toLowerCase()}/create/${
                        alarm.id
                      }`
                    )
                  "
            ...
            <template v-slot:severity="{ row: alarm }">
              <span v-html="NokiaTXNAlarmSeverityBadge(alarm.severity)"></span>
            </template>
  ...
  <alarm-emails
    ...
    :title="`${$nokiaTXNPresentationName} Alarm Email`"
    :htmlTemplate="
      alarmEmailService.getEmailTemplate('nokia-txn', alarmEmailData.alarmId)
    "
    :sendEmailFunction="
      (data) => {
...
import { NceTXNFilterConfig } from "./filter-config";
...
import {
  nokiaTXNActions,
  nokiaTXNMutations,
} from "@/store/enums/nokia-txn-enums";
import NokiaTXNAlarmDetailSide from "@/components/nokia-txn-components/NokiaTXNAlarmDetailSide.vue";
import NokiaTXNAlarmDetailModal from "@/components/nokia-txn-components/NokiaTXNAlarmDetailModal.vue";
...
export default defineComponent({
  ...
  setup() {
    const headerConfig = ref([
      ...
      {
        columnLabel: "Affected Object Name",
    ...
    const searchPlaceholder = `Search ${AppConstants.NOKIA_TXN_PRESENTATION_NAME} Alarms`;
    ...
    onUnmounted(() => {
      store.commit("enableNokiaTxnAlarmsBgMode");
    });
    ...
    onMounted(() => {
      if (unSeenAlarmNotificationCount.value > 0) {
        store.commit(
          ...
          nokiaTxnAlarmsCount.value
        );
      }
      store.commit("disableNokiaTxnAlarmsBgMode");
      store.commit("clearNokiaTxnAlarmsCount");
    });

    async function fetchData(queryParams?: QueryParams) {
      await store.dispatch(
        nokiaTXNActions.GET_ALL_NOKIA_TXN_ALARMS,
        queryParams
      );
      return store.getters.nokiaTXNAlarms;
    }

    function applyFilterDataManipulator(data) {
      store.commit(nokiaTXNMutations.SET_NOKIA_TXN_FILTERS, data);
      return data;
    }
    ...
    async function toggleEmailModal(alarm) {
      closeAllModal();
      alarmEmailData.value.subject = `[${alarm.ne_name}] - ${alarm.alarm_name}`;
      const recipients =
        await NokiaTXNAlarmService.getAlarmEmailRecipientsFromId(alarm.id);
    ...
    const setPlayAlarmSound = () => store.commit("toggleCanPlayNokiaTXNSound");
    return {
      headerConfig,
      searchPlaceholder,
      fetchData,
