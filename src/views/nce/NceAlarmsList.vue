<template>
  <crud-view-layout>
    <template v-slot:title-help>
      <span class="d-flex justify-content-center align-items-center">
        ...
        (Live Alarms from {{ $ncePresentationName }} Monitoring NMS)
        <button
        ...
        <div :class="{ 'col-md-8 border-end': rowAlarmId > 0 }">
          ...
          >
            <template #custom-actions>
              <TableColumnFiltration
                ...
                :tableColumnFiltrationKey="
                  AppConstants.TABLE_FILTER_COLUMN_KEYS.NCE_ALARM_LIST
                "
            ...
            <template v-slot:actions="{ row: alarm }">
              <div class="d-flex gap-2">
                ...
                <span
                  ...
                  @click="
                    $router.push(
                      `/trouble-ticket/${AppConstants.APP_TYPE.NCE.toLowerCase()}/create/${
                        alarm.id
                      }`
                    )
                  "
            ...
            <template v-slot:severity="{ row: alarm }">
              <span v-html="NceAlarmSeverityBadge(alarm.severity)"></span>
            </template>
  ...
  <alarm-emails
    ...
    :title="`${$ncePresentationName} Alarm Email`"
    :htmlTemplate="
      alarmEmailService.getEmailTemplate('nce', alarmEmailData.alarmId)
    "
    :sendEmailFunction="
      (data) => {
...
import { NceFilterConfig } from "./filter-config";
...
import NceAlarmDetail from "@/components/nce-components/NceAlarmDetailModal.vue";
  ...
  setup() {
    const headerConfig = ref([
      ...
      },
      {
        ...
        key: "severity",
        sortEnabled: true,
        boxColor: getSeverityCellColorByAppType(AppConstants.APP_TYPE.NCE),
      },
      {
      ...
      { key: "fiber_name", columnLabel: "(Fiber/Cable Name)" },
    ]);
    ...
    const searchPlaceholder = `Search ${AppConstants.NCE_PRESENTATION_NAME} Alarms`;
    ...
    onUnmounted(() => {
      store.commit("enableNCEAlarmsBgMode");
    });
    ...
    onMounted(() => {
      if (unSeenAlarmNotificationCount.value > 0) {
        store.commit(
          ...
          nceTxnAlarmsCount.value
        );
      }
      store.commit("disableNCEAlarmsBgMode");
      store.commit("clearNceTxnAlarmsCount");
    });

    async function fetchData(queryParams?: QueryParams) {
      await store.dispatch(nceActions.GET_ALL_NCE_ALARMS, queryParams);
      return store.getters.nceAlarms;
    }

    function applyFilterDataManipulator(data) {
      store.commit(nceMutations.SET_NCE_FILTERS, data);
      return data;
    }
    ...
    async function toggleEmailModal(alarm) {
      ...
      const recipients = await NceAlarmService.getAlarmEmailRecipientsFromId(
        alarm.id
      );
    ...
    const canPlayAlarmSound = computed(() => store.getters.getCanPlayNceSound);
    const setPlayAlarmSound = () => store.commit("toggleCanPlayNceSound");
    return {
      headerConfig,
      searchPlaceholder,
      fetchData,
