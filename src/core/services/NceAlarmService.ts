...
export default class NceAlarmService {
  public static async getAllAlarms(queryParms: QueryParams = {}) {
    queryParms = prepareDataForServer(queryParms);
    const response = await CurdApiService.getAll("nce-alarms", queryParms);
    return response.data.data;
  }

  public static async getAlarmById(id: string) {
    const response = await CurdApiService.getById("nce-alarms", id);
    return response.data.data;
  }

  // matches condition
  public static doesAlarmMatchesFilterConditions(Alarm: INceAlert) {
    const filters: QueryParams = removeEmptyStringValuesFromObject(
      proxyToObject(store.getters.nceFilters)
    );
